...
import type { CreateServiceDto } from '../services/services';
      ...
      const errorMessage =
        err && typeof err === 'object' && 'response' in err
          ? (err as { response?: { data?: { error?: string } } }).response?.data
              ?.error
          : undefined;
      setError(errorMessage || 'Failed to create service');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[60] flex items-start sm:items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-sm overflow-y-auto">
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-white flex-shrink-0">
          <div>
            <h2 className="text-xl font-bold text-gray-800">New Service</h2>
            <p className="text-sm text-gray-500 hidden sm:block">
              Add a new treatment or service to your catalog
            </p>
          </div>
          <button
            onClick={onClose}
